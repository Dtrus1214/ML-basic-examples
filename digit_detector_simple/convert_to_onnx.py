...
Usage:
  python convert_to_onnx.py --checkpoint model_best.pt --output model.onnx
  python convert_to_onnx.py --checkpoint model_best.pt --output model.onnx --dynamic  # batch dimension dynamic
...
def main():
    ...
    parser.add_argument("--checkpoint", type=str, default="model_best.pt", help="Path to .pt checkpoint")
    ...
    parser.add_argument("--hidden", type=int, default=128, help="Hidden size (must match trained model)")
    parser.add_argument("--dynamic", action="store_true", help="Use dynamic batch size (batch dim as variable)")
    args = parser.parse_args()

    device = torch.device("cpu")
    ckpt = torch.load(args.checkpoint, map_location=device)
    model = get_model(num_classes=10, hidden=args.hidden)
